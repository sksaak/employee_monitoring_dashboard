...
profiles = {
    'good': {
        ...
        'negative': lambda: expon.rvs(loc=0, scale=np.random.choice([.5, 1])).astype(int),  # noqa: 501
        'chance': .5
    },
    'normal': {
...
df = pd.DataFrame(data,
                  columns=['employee_id', 'team_id', 'event_date',
...
notes = pd.DataFrame(_,
                     columns=['employee_id', 'employee_name', 'note']).assign(
...
df = (df.merge(notes[['employee_id', 'event_date', 'note']],
               ...
               how='left')
      .merge(notes[['employee_id', 'employee_name']]
             .drop_duplicates(), on=['employee_id']))
...
X = (events.groupby('employee_id')[['positive_events', 'negative_events']]
     .sum())
y = X.join(df.drop_duplicates('employee_id').set_index('employee_id')
           [['recruited']]).recruited
